    ...
    var ensureIds = function (series) {
      ...
      return {
          ...
          link: function(scope, element, attrs) {
              ...
              scope.chart = initialiseChart(scope, element, scope.config);

              scope.$watch("config.series", function(newSeries, oldSeries) {
                  //do nothing when called on registration
                  if (newSeries === oldSeries) return;
                  processSeries(scope.chart, newSeries);
                  scope.chart.redraw();
              }, true);
              ...
              scope.$watch("config.xAxis", function(newAxes, oldAxes) {
                  ...
                  if (newAxes) {
                      scope.chart.xAxis[0].update(newAxes);
                      updateZoom(scope.chart.xAxis[0], angular.copy(newAxes));
                      scope.chart.redraw();
                  }
              }, true);
